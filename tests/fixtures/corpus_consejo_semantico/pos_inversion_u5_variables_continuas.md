...
- **Variables Aleatorias Discretas** (Unidad 3) — PMF, CDF y esperanza se generalizan al caso continuo reemplazando sumatorias por integrales; conviene tener presente la construcción discreta antes de este salto.
- **Distribuciones Conjuntas** (Unidad 4) — la noción de densidad marginal y condicional continua se apoya en la versión conjunta ya vista ahí.
- **Cálculo integral de una variable** — integrales definidas e impropias, para calcular probabilidades como áreas bajo la PDF y esperanzas como $\int x\, f(x)\, dx$.
...
(la desigualdad se invierte para $g$ cóncava). Intuitivamente, una función convexa "premia más" los valores alejados de la media de lo que "penaliza" los cercanos, por lo que promediar después de transformar da un resultado menor o igual que transformar el promedio.
