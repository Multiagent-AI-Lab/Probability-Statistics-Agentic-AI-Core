...
## 0.1 Política de Uso de IA del Curso

Este curso entrega al estudiante un tutor con IA (`StatsTutorAgent`, ver "Herramientas de esta Unidad" al final de cada unidad) desde el primer día. A diferencia de un curso de programación desde cero, aquí no hay una progresión de "sin IA para código → Copiloto → IA como arquitecto": el estudiante de 3er semestre ya sabe programar, y la IA que se le entrega no genera código de producción, sino que resuelve dudas conceptuales citando la fuente exacta. La política, entonces, es sobre **para qué sirve el tutor y qué no debe reemplazar**:

| Uso | Permitido | Detalle |
| :--- | :--- | :--- |
| **Dudas conceptuales** (¿por qué $n-1$? ¿qué significa este supuesto?) | **Sí, es el uso previsto.** | El tutor cita la sección exacta de la unidad y hace preguntas socráticas ante errores comunes, en vez de solo dar la respuesta. |
| **Depuración de código propio** | **Sí, con criterio propio.** | El tutor puede ayudar a interpretar un traceback, pero el estudiante debe entender y poder explicar la causa del error, no solo copiar el fix. |
| **Resolver la Autoevaluación / Ejercicio Propuesto** | **No.** | El bloque de autoevaluación de cada unidad se autocalifica contra un valor de referencia fijo — usar el tutor (o cualquier IA) para obtener la respuesta en vez de calcularla anula el propósito de esa celda. |
| **Interpretación nanotecnológica de un resultado** (Criterio 4 de `RUBRICA_GENERAL.md`) | **No como sustituto del razonamiento propio.** | El tutor puede ayudar a verificar si una interpretación es correcta, pero la rúbrica exige análisis propio del estudiante, no una interpretación generada y copiada. |
| **Exámenes y defensa oral** | **CERO IA.** | Igual que en el resto de cursos del programa: solo conocimiento propio, sin asistencia de ningún modelo. |

> [!TIP]
> El tutor está diseñado para no resolver el ejercicio por ti: si le pides directamente la respuesta a la Autoevaluación, su propio prompt lo orienta a redirigirte al concepto en vez de darte el número. Usarlo para eso de todas formas (pidiéndole el procedimiento completo y copiándolo) rompe la política aunque el tutor haya intentado evitarlo.
...
**Interpretación**: el Coeficiente de Variación ($CV = s/\bar{x}$) expresa la dispersión relativa a la escala de la media — a diferencia de $s$ (en nm), el CV es adimensional, lo que permite comparar la variabilidad de lotes de nanopartículas con diámetros promedio muy distintos (ver gráfico dedicado en la Sección 6.7). La Media Geométrica ($12.8670$ nm) es ligeramente mayor que la Media Aritmética ($12.8889$ nm) — una relación que se cumple siempre para datos positivos no idénticos (desigualdad AM-GM), y que se vuelve más pronunciada cuanto mayor es la dispersión relativa de los datos (ver comparación explícita en la Sección 6.10 con un dataset de tasas de rendimiento).
...
Notación general introducida en las Secciones 1-4, independiente del ejemplo aplicado específico:

* $x_1, x_2, \dots, x_n$: muestra observada de $n$ mediciones.
* $\bar{x}$: media aritmética muestral.
* $\tilde{x}$: mediana muestral.
* $R$: rango muestral ($x_{\max}-x_{\min}$).
* $s^2$: varianza muestral (con corrección de Bessel, denominador $n-1$).
* $s$: desviación estándar muestral ($s=\sqrt{s^2}$).
* $q$: cuantil de orden $q\in(0,1)$; $Q_1, Q_2, Q_3$: cuartiles (percentiles 25, 50 y 75).
* $IQR$: rango intercuartílico ($Q_3-Q_1$).
* $g_1$: coeficiente de asimetría (skewness).
* $g_2$: exceso de curtosis (kurtosis).
* $CV$: coeficiente de variación ($s/\bar{x}$), dispersión relativa adimensional.
* $f_h(x)$: estimador de densidad por kernel (KDE), con ancho de banda $h$ y kernel $K(u)$.
