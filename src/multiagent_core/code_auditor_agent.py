"""
CodeAuditorAgent: Audits Python code for statistical correctness, PEP8, display(Math()) usage, and best practices.
"""
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Import):
                        for alias in node.names:
                            if "scipy" in alias.name: metrics["uses_scipy"] = True
                            if "statsmodels" in alias.name: metrics["uses_statsmodels"] = True
                            if "sympy" in alias.name: metrics["uses_sympy"] = True
                    elif isinstance(node, ast.ImportFrom):
                        if node.module:
                            if "scipy" in node.module: metrics["uses_scipy"] = True
                            if "statsmodels" in node.module: metrics["uses_statsmodels"] = True
                            if "sympy" in node.module: metrics["uses_sympy"] = True
                        ...
                        if func_name == "display":
                            ...
                            for arg in node.args:
                                if isinstance(arg, ast.Constant) and isinstance(arg.value, str):
                                    if r"\frac" in arg.value or r"\mu" in arg.value or r"\sigma" in arg.value:
                                        metrics["uses_raw_print_latex"] = True
                                        warnings.append("Avoid print() for LaTeX equations; use display(Math()) instead.")
