...
| **Fase 2: Análisis Exploratorio (EDA)** | Estadísticos descriptivos de su propio dataset, prueba de Shapiro-Wilk y Levene, al menos un gráfico exploratorio (histograma o boxplot) | Notebook con celdas ejecutadas: $\bar{X}$, $S^2$, IQR por grupo + `stats.shapiro` + `stats.levene` con sus $p$-valores impresos + ≥1 figura | Semana 6-7 (tras U3-U4, con distribuciones y variables aleatorias ya vistas) |
| **Fase 3: Verificación Simbólica en SymPy** | Deriva en SymPy la expresión exacta del estadístico de prueba que va a usar (no solo la ejecuta con SciPy: la deriva primero) | Celda de SymPy que define los símbolos, construye la fórmula del estadístico (p. ej. $t$, $F$, o $\chi^2$ según la prueba elegida) y evalúa la expresión simbólica con los valores numéricos de su propio dataset — coherente con el Ciclo de Verificación Triple de `GOVERNANCE.md` | Semana 10-11 (tras U6, con el Ciclo de Verificación Triple ya practicado varias veces) |
| **Fase 4: Solución Computacional** | Ejecuta la prueba de hipótesis con SciPy/statsmodels sobre su propio dataset y simula la curva de potencia por Monte Carlo | Notebook con `scipy.stats.ttest_ind`/`f_oneway`/equivalente + gráfico de la curva de potencia simulada, con el resultado simbólico de la Fase 3 contrastado numéricamente contra este resultado computacional (tolerancia relativa razonable, no exacta — son dos rutas de cómputo distintas) | Semana 13-14 (tras U7, con la metodología completa de pruebas de hipótesis ya vista) |
...
- **Fase 2**: el notebook contiene al menos una llamada a `stats.shapiro` **y** una a `stats.levene` (o su justificación explícita de por qué no aplican, p. ej. una sola muestra) — contiene al menos 1 figura (`plt.`/`sns.`) — reporta medias y varianzas/desviaciones estándar de cada grupo.
- **Fase 3**: el notebook usa `sympy` (import y al menos un `sp.Symbol`/`sp.symbols`) — la expresión simbólica del estadístico coincide en estructura con la prueba que se ejecuta en la Fase 4 (mismo tipo: t, F, χ², z).
...
| **2. Análisis Exploratorio** | Faltan Shapiro-Wilk o Levene sin justificación, o no hay gráfico | Supuestos verificados y graficados, pero sin discutir qué implican para la elección de prueba en Fase 4 | Supuestos verificados, discutidos, y ya anticipan explícitamente la prueba (paramétrica o no paramétrica) que se usará |
| **3. Verificación Simbólica (SymPy)** | No hay derivación simbólica, o es solo una copia de la fórmula sin sustituir los valores propios | La expresión simbólica es correcta y se evalúa con los datos propios | La derivación simbólica está comentada paso a paso y se contrasta explícitamente contra el resultado de SciPy de la Fase 4 |
